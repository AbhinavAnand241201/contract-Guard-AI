...
export function ContractRiskScannerFeature() {
  ...
  const getRiskLevelColorClass = (level: 'Low' | 'Medium' | 'High' | undefined) => {
    ...
    if (level === 'Medium') return 'text-yellow-500 dark:text-yellow-400 font-bold'; 
    if (level === 'Low') return 'text-green-500 dark:text-green-400 font-bold';
    ...
    <Card className="w-full shadow-xl rounded-xl overflow-hidden card-hover-effect">
      <CardHeader className="border-b p-4 sm:p-6 bg-card/50"> 
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <CardDescription className="text-sm text-muted-foreground">Upload your contract (PDF, DOCX, TXT) to identify potential risks and get an AI-powered analysis.</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="p-4 sm:p-6 space-y-6">
        <div className="space-y-2">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 p-4 border-2 border-dashed rounded-lg hover:border-primary transition-colors duration-300 bg-background">
            <UploadCloud className="w-10 h-10 text-muted-foreground sm:mb-0 mb-2 shrink-0" />
            <Input
              id="contract-file"
              type="file"
              onChange={handleFileChange}
              className="flex-grow file:mr-4 file:py-2 file:px-4 file:rounded-md file:border file:border-input file:bg-primary/10 file:text-sm file:font-semibold file:text-primary hover:file:bg-primary/20 cursor-pointer transition-colors"
        ...
        {analysisResult && !isLoading && (
          <div className="space-y-6 pt-6 border-t mt-6">
            <h3 className="text-xl sm:text-2xl font-semibold font-headline text-primary mb-2">Analysis Results</h3>
            ...
            <Card className="bg-card border rounded-lg shadow-md">
              ...
              <CardContent className="px-4 sm:px-5 pb-4 space-y-2">
                <p className={`text-2xl sm:text-3xl ${getRiskLevelColorClass(analysisResult.riskLevel)}`}>
            ...
            <Card className="bg-card border rounded-lg shadow-md">
              ...
              <CardContent className="px-4 sm:px-5 pb-4">
                <div className="text-sm whitespace-pre-wrap bg-muted/50 p-3 sm:p-4 rounded-md max-h-96 overflow-y-auto border">
      ...
      <CardFooter className="border-t p-4 sm:p-6 bg-muted/50">
        <Button onClick={handleSubmit} disabled={isLoading || !file} className="w-full sm:w-auto text-base py-3 px-6 bg-primary hover:bg-primary/90 text-primary-foreground button-hover-effect shadow-md hover:shadow-lg">
          {isLoading && loadingText ? (
            <>
